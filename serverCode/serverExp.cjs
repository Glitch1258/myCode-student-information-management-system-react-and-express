...
app.post("/createNewEntry", (req, res) => {
  try {
    QueryFunctions.getEntryById(req.body.id, databaseConnection, (err, result) => {
      if (err) {
        console.log(err);
      ...
      if (result) {
        res.status(400).send("An entry with the same ID already exists. ID needs to be unique.");
        return;
      } else {
